...
if selected == "Author":
    ...
    st.write("**Viêm phổi** là một bệnh đe dọa tính mạng, xảy ra ở phổi do nhiễm vi khuẩn hoặc virus. Nó có thể gây nguy hiểm đến tính mạng nếu không được hành động đúng lúc và do đó việc chẩn đoán sớm bệnh viêm phổi là rất quan trọng. Mục đích của bài báo này là tự động phát hiện viêm phổi bằng cách sử dụng hình ảnh X – ray. Bằng việc sử dụng các model train trên Google Colab, mô hình đạt 93% độ chính xác, cho ra kết quả dự đoán khá chính xác.")
    st.write("Các dữ liệu model training sẽ được đưa lên webApp Streamlit, để thao tác cũng như hiển thị kết quả một cách dễ dàng hơn.")
